...
export function createServer() {
  ...
  if(devToolsEnabled) {
    server.registerTool(
      "list_tables",
      {
        description: "DEV ONLY: list all tables in the boxwood-ams360 database, grouped by schema. Not available in production.",
        inputSchema: {}
      },
      async () => {
        try {
    ...
    server.registerTool(
      "describe_table",
      {
        description: "DEV ONLY: describe the columns of a table, including type, nullability, default, and primary key membership. Not available in production.",
        inputSchema: {
    ...
    server.registerTool(
      "run_query",
      {
        description: "DEV ONLY: run a read-only SELECT query against boxwood-ams360 and return the resulting rows. Not available in production.",
        inputSchema: {
