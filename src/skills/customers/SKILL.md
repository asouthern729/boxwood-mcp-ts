...
| `certificates` | `afw_certliabprop` + `afw_certholderinfo` | Lightweight per-certificate view (current holder only) — use `certificate_lookup` for full reissue history or covered-policies detail |
| `policies` | `afw_basicpolinfo` + `afw_cprem` | Lightweight view of the customer's *current* (in-force today) policies — policy #, carrier, dates, premium. Same "current" definition as `policy_query`'s `renewalrptflag: "A"` filter and `book_summary`; use `policy_query` directly (`custid` filter) for expired/prior terms, or its `premiums` include for coverage-line-level pricing detail |
...
- **`mastersubtype`/`mastercustid`** encode Multiple Entity setups — a sub-customer's `mastercustid` points back to the master. `mastersubtype` resolves to `mastersubtype_description` as of 2026-09-01 (via `afw_prcode`, `AttrCode='ME'`): `D`=Standard (not part of a Multiple Entity setup — the overwhelming majority), `M`=Master, `S`=Sub-customer. Useful for "who are the related entities on this commercial account."
- **`active` and most other single-char fields are still raw AMS360 passthrough codes, not enums** — treat any single-char field as "whatever the source system sent," not a fixed list to validate against, unless noted otherwise below.
- **`typecust` is NOT a commercial/personal flag** (an earlier version of this note guessed wrong) — it's a customer-relationship-status code hardcoded in the AMS360 Database Design Guide's prose (not `afw_prcode`-backed): `C`=Customer (1,789 of 3,025 in this book), `P`=Prospect (1,236), `S`=Suspect (not present in this tenant's data). Resolves to `typecust_description` on the core result as of 2026-09-01 via a join to `_code_lookup` (`category='typecust'`) — a small hand-maintained table for Guide-hardcoded-only codes, see the `policies` skill for the full explanation — confirmed against the Guide text with `postgres-mcp-9f`, not guessed from observed values.
...
- "What does this customer's current book look like — policies and premium?" → `include: ["policies"]`; for coverage-line pricing detail on one of those policies, follow up with `policy_query(polid=..., include: ["premiums"])`
