...
const accuracyVerdictSchema = z.object({
  ...
  note: z.string().describe("One or two sentences citing what in candidate_prior_activity supports (or fails to support) this verdict. This text is shown to reps verbatim in the finished workbook's \"Check for Accuracy\" column — write it as a human-readable explanation, not tool output, and never reference an item_id or any other internal identifier (a rep has no way to look one up — it appears nowhere in the workbook). Cross-referencing another item on this same report is still valuable and should still be done whenever it's genuinely true — e.g. noticing the same change is documented on a different transaction for this policy — just identify that other item by its content instead of its number (the customer, the policy number, the date, or what it actually says, e.g. 'the same Colorado truck removal noted on this policy's other transaction'). Don't drop a real cross-reference just to avoid an item_id — rephrase it, don't omit it")
})
