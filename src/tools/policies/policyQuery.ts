  ...
  server.registerTool(
    "policy_query",
    {
      ...
      inputSchema: {
        ...
        status: z.string().length(1).describe("Exact match against raw AMS360 status code (afw_basicpolinfo.status). NOT a reliable signal for \"current/active\" — despite the name, it doesn't track renewal-chain lifecycle in this data. Use renewalrptflag for that instead. When omitted, results already exclude status='D' (deleted/superseded) rows by default — pass status explicitly (including 'D' itself) to see them.").optional(),
        renewalrptflag: z.string().length(1).describe("Pass \"A\" to get only the currently in-force term of each policy/renewal chain — this is the correct filter for \"current\"/\"active\" policies, not status. \"A\" is resolved by effective/expiration dates, NOT a literal match against the raw renewalrptflag column (see tool description) — also excludes submission shells and status='D' rows. Any other value (e.g. \"R\", \"C\") is an exact, literal match against the raw column instead, with none of those extra exclusions.").optional(),
      ...
      try {
        ...
        if(polid) {
          ...
          if(status) {
            ...
            // Deleted (status='D') rows are stale/superseded duplicates, not real results — confirmed
            // against real data that afw_basicpolinfo can carry a deleted row and a current row for
            // the same polno with DIFFERENT csrcode/execcode (e.g. a CSR correction), so a bare
            // csr_code/carrier_code/polno search with no explicit status filter would otherwise surface
            // the stale row right alongside the current one (client-reported 2026-09-08: "find a
            // commercial account for [CSR]" returned an account that had since moved to another CSR).
            // Excluded by default unless the caller explicitly asks for a specific status (including
            // 'D' itself, to intentionally inspect deleted history).
            conditions.push(`p.status != 'D'`)
          }
          if(renewalrptflag === "A") {
            ...
            // renewalrptflag='A' would miss exactly that term. The date bounds below are what
            // actually define "current" (mirrors book_summary's current_policies definition — a
            // bound renewal with a future poleffdate, or a term whose polexpdate has already passed,
            // doesn't count as current) — no equality condition on the raw column at all.
            conditions.push(
              `p.polsubtype != 'S'`,
              `p.status != 'D'`,
              `p.poleffdate <= now()`,
          ...
          } else if(renewalrptflag) {
            // Any other renewalrptflag value is a literal, raw-column filter — a different,
            // non-"current" question — so none of the extra "A" exclusions apply.
