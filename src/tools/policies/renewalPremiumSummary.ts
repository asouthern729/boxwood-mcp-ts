...
export function registerRenewalPremiumSummaryTool(server: McpServer) {
  server.registerTool(
    ...
    async ({ custid, renewal_within_days }) => {
      try {
        ...
        return textResult({
          message: `Built the Renewal Premium Summary for ${ result.clientName } — ${ result.includedCount } polic${ result.includedCount === 1 ? "y" : "ies" } renewing within ${ renewal_within_days } days, ${ result.otherCount } other current polic${ result.otherCount === 1 ? "y" : "ies" } listed separately.`,
        ...
        logger.error({ err: error, custid, renewal_within_days }, "renewal_premium_summary failed")
