...
export function registerBoardUpdateItemTool(server: McpServer) {
  server.registerTool(
    "board_update_item",
    {
      description: "Update one item on the Boxwood/Tyneside tool roadmap (site/roadmap.html) — its name, status, priority level, and/or notes. Identify the item by id (exact, from board_summary) or name (partial match — errors listing the candidates if more than one matches). At least one of new_name/status/priority/notes is required. priority is 1/2/3 or null (pass null to clear it). Use this after finishing work on something (e.g. set status to completed), to reprioritize, or to rename an item — this is the same board the client sees and comments on, so a change here is visible to them immediately.",
      inputSchema: {
        ...
        new_name: z.string().describe("Renames the item to this").optional(),
    ...
    async ({ id, name, new_name, status, priority, notes }) => {
      try {
        if(new_name === undefined && status === undefined && priority === undefined && notes === undefined) {
          throw new Error("Provide at least one of new_name, status, priority, or notes to update")
        }
        ...
        logger.error({ err: error, id, name, new_name, status, priority, notes }, "board_update_item failed")
